...
public class NacosNamingService implements NamingService {
    ...
    private void init(Properties properties) throws NacosException {
        ...
        //这里会创建一个NamingClientProxyDelegate
    ...
    public List<Instance> getAllInstances(String serviceName, String groupName, List<String> clusters,
        ...
        //在这里其实发现了，从远程拉了数据没有更新到本地，这里没有更新到本地，本地的更新会有一个定时任务，定时从远程拉去更新
        if (subscribe) { //是否订阅
            //订阅，看下这个方法
            serviceInfo = serviceInfoHolder.getServiceInfo(serviceName, groupName, clusterString);
            //如果从本地拉不到服务信息，那么调用subscribe方法
            if (null == serviceInfo) {
                serviceInfo = clientProxy.subscribe(serviceName, groupName, clusterString);
            }
        } else {
            //不进行订阅,从方法上可以看出就是直接从server中获取serviceInfo，这里注意这个udpPort传入0
    ...
    /**
     * 在之前只是使用了NamingService的getAllInstances 方法作为了服务发现获取服务实例列表的方式，其实nacos还支持订阅方式，
     * 通过提供订阅的服务名称与监听器（listener ）就可以了，然后订阅的服务实例信息一旦发生变化，nacos服务端就会通知你这个client，
     * client收到通知后就会调用你这监听器，执行对应的逻辑。
    ...
    /**
     * ，直接调用subscribe 的方法就对该服务订阅了，一旦“orderService”服务实例信息发生变化，nacos服务端就会使用udp协议通知你这个client，
     * client收到通知后，判断与本地缓存的服务实例信息有没有变化，如果有变化的话，就会调用你这个事件监听器。
