...
public class NamingHttpClientProxy extends AbstractNamingClientProxy {
    ...
    public NamingHttpClientProxy(String namespaceId, SecurityProxy securityProxy, ServerListManager serverListManager,
        ...
        //这里会初始化一个BeatReactor，这个其实就是用来发心跳的，看他的构造方法
    ...
    public void registerService(String serviceName, String groupName, Instance instance) throws NacosException {
        ...
        //如果是临时节点，然后就会封装beatInfo,添加到beatReactor组件中
        if (instance.isEphemeral()) { //如果是临时性质的，这个是为了发送心跳
            //创建心跳信息，也就是续约信息
            BeatInfo beatInfo = beatReactor.buildBeatInfo(groupedServiceName, instance);
            //添加到beatReactor,看下addBeatInfo方法
    ...
    /**
     * 如果是临时节点的话，就会调用beatReactor这个组件移除beatInfo这个任务，就是不用再发送心跳续约了。接着就是调用serverProxy这个组件的deregisterService 方
     * 法进行服务下线，这里需要注意的是NamingUtils.getGroupedName(serviceName, groupName)
    ...
    public void deregisterService(String serviceName, String groupName, Instance instance) throws NacosException {
        ...
        //发送服务下线的请求,请求的uri是/nacos/v1/ns/instance
    ...
    //这里直接是调用了serverProxy组件的list 方法，需要主要的是这个udp端口是0 ，因为咱们这里是不订阅的，这个upd是给订阅的接收通知用的
    //直接看服务端的list方法
    @Override
     ...
     *                        这里就是封装请求参数，调用reqApi 方法进行发送心跳请求，这里lightBeatEnabled 这个参数明白了，
     *                        就是请求的时候要不要将 beatInfo带过去，默认是带过去的，请求uri是/nacos/v1/ns/instance/beat ，
     *                        然后请求方式是put，往下我们就不看了，其实就是找哪个server进行发送，与服务注册，服务下线是一样的，先随机找一个server
     *                        ，如果失败的话就下个server轮询发送，重试次数不超过server的个数。
     */
    public JsonNode sendBeat(BeatInfo beatInfo, boolean lightBeatEnabled) throws NacosException {
        ...
        //put请求方式 ，我们看下服务端的/instance/beat方法
    ...
    public ServiceInfo subscribe(String serviceName, String groupName, String clusters) throws NacosException {
        //这里也是调用queryInstancesOfService，只不过这里的udpPort不是0了。
    ...
    public String reqApi(String api, Map<String, String> params, Map<String, String> body, List<String> servers,
        ...
        //失败重试，多个service的时候会随机选一个，然后如果失败，轮询下一个，有多少个service就会重拾多少次
     ...
     * @return result
     * @throws NacosException nacos exception
     *                        这里组装url并调用nacosRestTemplate发送请求
     */
